...
def get_data():
    ...
    df['stck_cntg_hour'] = pd.to_datetime(
        df['stck_cntg_hour'], format='%H%M%S').dt.time
    ...
    df.index = pd.to_datetime(df['stck_bsop_date'].astype(
        str) + ' ' + df['stck_cntg_hour'].astype(str))
...
def get_company_data():
    ...
    company_info = company_row[['단축코드', '한글명', '기준가']].to_dict(orient='records')[
        0]
